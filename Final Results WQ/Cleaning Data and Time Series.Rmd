...
---
 Organized Data Sets to Use (from this document):
MCByDay, SIByDay, 
...
2015MCCal.csv, 2015SICal.csv, 2018SICal.csv
2015MCCalDay.csv, 2015SICalDay.csv, 2018SICalDay.csv
...
```{r Fixing the dates}
Working<- read.csv("MC2018Day.csv")

Working$Date<- ymd(Working$Date)

write.csv(Working, file="MC2018Day.csv", row.names = FALSE)

#-------------------------------

Working<- read.csv("MC2017Day.csv")

Working$date<- ymd(Working$date)

write.csv(Working, file="MC2017Day.csv", row.names = FALSE)
#-------------------------------

Working<- read.csv("MC2016Day.csv")

colnames(Working)[1]<- "Date"

Working$Date<- ymd(Working$Date)

write.csv(Working, file="MC2016Day.csv", row.names = FALSE)

#--------------------------------

Working<- read.csv("MC2015Day.csv")

Working$Date<- ymd(Working$Date)

write.csv(Working, file="MC2015Day.csv", row.names = FALSE)

#=================================

Working<- read.csv("2018MC.csv")

Working<- Working %>%mutate(HourProper= hour)

Working$HourProper<-  paste(Working$hour, ':00:00')

Working<- Working %>% mutate(DateHour= mdy(samp.date) + hms(HourProper)) %>% rename(Hour= hour, Day= day, Month= month) %>% select(DateHour, Date, Hour, Month, Day, Turb, CHLugL, BGAugL, FDOMqsu, NO3_mgL,Discharge) 

Working$Date<- ymd(Working$Date)

write.csv(Working, file="MC2018.csv", row.names = FALSE)

#---------------------------------

Working<- read.csv("2017MC.csv")

Working<- Working %>%mutate(HourProper= hour)

Working$HourProper<-  paste(Working$hour, ':00:00')

Working<- Working %>% mutate(DateHour= ymd(date) + hms(HourProper)) %>% rename(Hour= hour, Day= day, Month= month, Date=date) %>% select(DateHour, Date, Hour, Month, Day, Turb, CHLugL, BGAugL, FDOMqsu, NO3_mgL,Discharge) 

Working$Date<- ymd(Working$Date)

write.csv(Working, file="MC2017.csv", row.names = FALSE)

#---------------------------------

Working<- read.csv("2016MC.csv")

Working<- Working %>%mutate(HourProper= NormHour)

Working$HourProper<-  paste(Working$NormHour, ':00:00')

Working<- Working %>% mutate(DateHour= ymd(date) + hms(HourProper)) %>% rename(Hour= NormHour, Day= day, Month= month, Date=date) %>% select(DateHour, Date, Hour, Month, Day, Turb, CHLugL, BGAugL, FDOMqsu, NO3_mgL,Discharge) 

Working$Date<- ymd(Working$Date)

write.csv(Working, file="MC2016.csv", row.names = FALSE)

#---------------------------------

Working<- read.csv("2015MCBiHour.csv")
...
write.csv(Working, file="MC2015.csv", row.names = FALSE)

#==================================

Working<- read.csv("SI2018BiHourly.csv")
...
write.csv(Working, file="SI2018B.csv", row.names = FALSE)

#-----------------------------------

Working<- read.csv("SI2016BiHourly.csv")
...
write.csv(Working, file="SI2016B.csv", row.names = FALSE)

#-----------------------------------

Working<- read.csv("SI2015BiHourly.csv")
...
write.csv(Working, file="SI2015B.csv", row.names = FALSE)
```
...
Below, different calibrations were idenitified and handled (places where the values would suddenly drop, and then rise again after approximately 1 month)
To handle the FDOM and nitrate data, points that were problematic were replaced using the mice function. This was because the data was so problematic (ie there were some points that were in the 50s, while others were below 0. Increasing the median would not have been effective). This covered almost a month of data in 2015
To handle the BGA data, a set value was added to all of the data that was lower, so that the median matched. This was possible because it was just a lower median followedby a higher median with set point were it dropped and raised, but the same sd
```{r Handling Dif calibrations}
#FDOM MC 2015
MC.2015ts<- read.csv("MC2015.csv")
 MC.2015ts[,9][MC.2015ts[,9]<95]<- NA
 MC.2015ts1<- complete(mice(MC.2015ts, m=1, maxit=50, method='pmm'))
 MC.2015.test<- MC.2015ts1 %>% group_by(Date) %>% summarize(Turb= mean(Turb), CHLugL= mean(CHLugL), BGAugL= mean(BGAugL), 
                                                           FDOMqsu=mean(FDOMqsu), NO3_mgL=mean(NO3_mgL),
                                                           Discharge=mean(Discharge))
 MC.2015.test$Date<- ymd(MC.2015.test$Date) #DONE
 ggplot(data=MC.2015.test)+ geom_line(mapping= aes(x=Date, y= FDOMqsu))
 MC.2015.ts1<-ts(MC.2015ts1, freq = 12)
 p15<-plot(MC.2015.ts1arg[, c(6, 7, 8, 9, 10)], main= "Water Quality in 2015")
 
 #FDOM SI 2015
 SI.2015ts<- read.csv("SI2015B.csv")
 SI.2015ts<- SI.2015ts %>% filter (!(month==10 & day==2 & hour >20)) %>% filter(!(month==10 & day>2))
  SI.2015ts[,9][SI.2015ts[,9]<81]<- NA
  SI.2015ts[348:719,10]<- NA #Nitrate
  SI.2015ts1<- SI.2015ts
 
 #BGA
 median(SI.2015ts[1:347,8])
 median(SI.2015ts[c(348:718),8])
  median(SI.2015ts[719:1128,8])
  addto1<-median(SI.2015ts[c(348:718),8])-median(SI.2015ts[1:347,8])
  addto2<-median(SI.2015ts[c(348:718),8])-median(SI.2015ts[719:1128,8])
   median(SI.2015ts[1129:1479,8])

    SI.2015ts1[1:348,8]<-(SI.2015ts[1:348,8] + addto1)
   SI.2015ts1[719:1128,8]<-(SI.2015ts[719:1128,8] + addto2) 
   median(SI.2015ts1[,8])
    

  SI.2015ts1<- complete(mice(SI.2015ts1, m=1, maxit=50, method='pmm'))
  
  SI.2015.ts1<-ts(SI.2015ts1, freq = 12)
 plot(SI.2015.ts1[, c( 7, 8, 9, 10, 11)], main= "Water Quality in 2015")
 
 SI.2015.test<- SI.2015ts1 %>% group_by(Date) %>% summarize(Turb= mean(Turb), CHLugL= mean(CHLugL), BGAugL= mean(BGAugL), 
                                                           FDOMqsu=mean(FDOMqsu), NO3_mgL=mean(NO3_mgL),
                                                           Discharge=mean(Discharge))
 SI.2015.test$Date<- ymd(SI.2015.test$Date) #DONE
 ggplot(data=SI.2015.test)+ geom_line(mapping= aes(x=Date, y= NO3_mgL))
 
 #SI 2018 Nitrate
 SI.2018ts<- read.csv("SI2018B.csv")
 SI.2018ts[,10][SI.2018ts[,10]<0.1]<- NA #Replace with NA
 SI.2018ts1<- complete(mice(SI.2018ts, m=1, maxit=50, method='pmm'))
  SI.2018.test<- SI.2018ts1 %>% group_by(Date) %>% summarize(Turb= mean(Turb), CHLugL= mean(CHLugL), BGAugL= mean(BGAugL), 
                                                           FDOMqsu=mean(FDOMqsu), NO3_mgL=mean(NO3_mgL),
                                                           Discharge=mean(Discharge))
 
 write.csv(MC.2015ts1, file="2015MCCal.csv", row.names = FALSE)
 write.csv(SI.2015ts1, file="2015SICal.csv", row.names = FALSE)
 write.csv(SI.2018ts1, file="2018SICal.csv", row.names = FALSE)
 write.csv(MC.2015.test, file="2015MCCalDay.csv", row.names = FALSE)
 write.csv(SI.2015.test, file="2015SICalDay.csv", row.names = FALSE)
 write.csv(SI.2018.test, file="2018SICalDay.csv", row.names = FALSE)
 ?mice
```
